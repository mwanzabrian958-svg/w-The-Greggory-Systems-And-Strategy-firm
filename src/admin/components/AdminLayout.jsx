...
const BLOG_POSTS = [
  {
    ...
    excerpt:
      "Exploring the cutting-edge technologies shaping our future in web development and AI.",
    content:
      "In this comprehensive article, we dive deep into the latest technology trends that are transforming the digital landscape in 2024. From artificial intelligence and machine learning to advanced web frameworks and cloud computing, discover how these innovations are reshaping how we build and interact with technology. We explore practical applications, emerging tools, and provide insights on how businesses can leverage these trends to stay competitive in the rapidly evolving tech ecosystem.",
  },
  {
    id: 2,
  ...
  },
  {
    ...
    excerpt:
      "A comprehensive guide to building responsive websites with React and Tailwind.",
    content:
      "This step-by-step tutorial takes you through modern web development practices using the latest technologies. Learn how to build fast, responsive, and accessible websites using React, Tailwind CSS, and modern JavaScript. We cover everything from setting up your development environment to deploying your application. Includes practical examples, best practices for component design, state management techniques, and tips for optimizing performance. Perfect for developers looking to upgrade their skills or transition to modern web development.",
  },
  {
    id: 4,
  ...
  },
  {
    ...
    content:
      "The e-commerce landscape in Kenya is rapidly growing, presenting enormous opportunities for entrepreneurs and established businesses alike. This detailed guide walks you through every aspect of building a successful online store, from choosing the right platform and payment gateways to inventory management and customer service. We cover logistics challenges specific to the Kenyan market, strategies for building trust with online shoppers, and techniques for driving traffic and converting visitors into customers. Includes a checklist for launching your e-commerce business.",
  },
];
...
export function AdminLayout({ user, onLogout }) {
  ...
  const [activeProjectSubItem, setActiveProjectSubItem] =
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`/api/admin/${activeItem}/create`, {
        method: "POST",
        headers: {
  ...
  const getActiveItemLabel = () => {
    ...
    if (activeItem === "projects") {
      const subItem = PROJECT_SUB_ITEMS.find(
        (i) => i.id === activeProjectSubItem,
      );
            ...
            <nav className="flex items-center space-x-2 overflow-x-auto flex-1">
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`w-4 h-4 mr-2 ${isActive ? "text-white" : "text-gray-400"}`}
                    />
                    {item.label}
                  </button>
                );
              ...
              <div className="flex items-center space-x-3 bg-slate-800 rounded-lg px-3 py-2 shadow-md">
                ...
                {(() => {
                  ...
                  const photoUrl = user?.id
                    ? `/api/admin/profile-photo/${role}/${user.id}`
                    : null

                  const initials = (
                    (user?.first_name?.[0] || '') +
                    (user?.last_name?.[0] || '')
                  ).toUpperCase() || user?.display_name?.[0]?.toUpperCase() || user?.email?.[0]?.toUpperCase() || '?'

                  if (photoUrl) {
                    return (
                      <img
                        src={photoUrl}
                        alt={user?.display_name || user?.first_name || 'User'}
                        className="w-8 h-8 rounded-full object-cover border-2 border-slate-600 shadow-lg"
                        onError={(e) => {
                ...
                {/* Fallback initials div (hidden by default, shown if img errors) */}
                <div
                  className="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full items-center justify-center text-white text-sm font-bold shadow-lg hidden"
                >
                  {(
                    (user?.first_name?.[0] || '') +
                    (user?.last_name?.[0] || '')
              ...
              <div className="flex flex-wrap gap-3">
                {PROJECT_SUB_ITEMS.map((subItem) => {
                  ...
                  return (
                    ...
                    >
                      <Icon
                        className={`w-4 h-4 mr-2 ${isSubActive ? "text-white" : "text-slate-500"}`}
                      />
                      {subItem.label}
                    </button>
                  );
          ...
          {showAddForm && (
            <div className="bg-gray-50 rounded-lg border border-gray-200 p-6 mb-6">
              <h3 className="text-lg font-semibold text-gray-700 mb-4">
                Add New {getActiveItemLabel()} Item
              </h3>
              ...
              {activeItem === "blog" && (
                ...
                >
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-600 mb-1">
                      Featured Image (Upload from Local Storage)
                    </label>
                ...
                activeProjectSubItem === "tracking" && (
                  ...
                  >
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea
                        name="notes"
                        placeholder="Latest updates and notes..."
                        rows="2"
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm"
                      />
                    </div>
          ...
          <div className="bg-gray-50 rounded-lg border border-gray-200 min-h-[200px] p-6">
            ...
            {activeItem === "blog" && selectedBlog && (
              <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                ...
                <div className="p-6 md:p-8">
                  <span
                    className={`inline-block px-3 py-1 text-sm font-semibold rounded mb-4 ${
                      selectedBlog.category === "Technology"
                        ? "bg-blue-100 text-blue-800"
                        : selectedBlog.category === "Business"
                          ? "bg-green-100 text-green-800"
                          : selectedBlog.category === "Tutorial"
                            ? "bg-purple-100 text-purple-800"
                            : "bg-yellow-100 text-yellow-800"
                    }`}
                  >
            ...
            {activeItem !== "blog" && activeItem !== "projects" && (
              <p className="text-gray-500 text-center">
                {activeItem === "home" &&
                  "Home page content items will be listed here..."}
                {activeItem === "about" &&
                  "About Us content will be listed here..."}
                {activeItem === "services" &&
                  "Our Services content will be listed here..."}
                {activeItem === "case-studies" &&
                  "Case Studies will be listed here..."}
              </p>
            )}
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {DOCUMENTATION_DATA.filter(
                          (item) =>
                            !filteredProject ||
                            item.project === filteredProject,
                        ).map((item) => (
                          <tr key={item.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-2 text-sm">
                              <span
                                className={`px-2 py-1 text-xs rounded ${item.status === "Active" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}
                              >
                                {item.status}
                              </span>
                            </td>
                    ...
                    {filteredProject &&
                      DOCUMENTATION_DATA.filter(
                        (item) => item.project === filteredProject,
                      ).length === 0 && (
                        <p className="text-gray-500 text-center py-4">
                          No documentation found for {filteredProject}
                        </p>
                      )}
                  </div>
                )}
                  ...
                  <div className="overflow-x-auto">
                    ...
                    {filteredProject &&
                      ACCOUNTING_DATA.filter(
                        ...
                        <p className="text-gray-500 text-center py-4">
                          No accounting records found for {filteredProject}
                        </p>
                      )}
                  </div>
                )}
                  ...
                  <div className="overflow-x-auto">
                    ...
                    {filteredProject &&
                      MPESA_DATA.filter(
                        ...
                        <p className="text-gray-500 text-center py-4">
                          No M-pesa transactions found for {filteredProject}
                        </p>
                      )}
                  </div>
                )}
                  ...
                  <div className="overflow-x-auto">
                    ...
                    {filteredProject &&
                      BANK_DATA.filter(
                        ...
                        <p className="text-gray-500 text-center py-4">
                          No bank records found for {filteredProject}
                        </p>
                      )}
                  </div>
                )}
                ...
                {activeProjectSubItem === "tracking" && (
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {TRACKING_DATA.filter(
                          (item) =>
                            !filteredProject || item.name === filteredProject,
                        ).map((item) => (
                          <tr key={item.id} className="hover:bg-gray-50">
                            <td className="px-4 py-2 text-sm text-gray-900 font-medium">
                              {item.name}
                    ...
                    {filteredProject &&
                      TRACKING_DATA.filter(
                        (item) => item.name === filteredProject,
                      ).length === 0 && (
                        <p className="text-gray-500 text-center py-4">
                          No tracking data found for {filteredProject}
                        </p>
                      )}
                  </div>
                )}
