...
export function Login({ onLoginSuccess }) {
  ...
  return (
    <div className="min-h-screen w-full bg-[#020617] flex flex-col lg:flex-row font-sans relative overflow-hidden">
      ...
      <aside className="relative z-10 w-full lg:w-[42%] xl:w-[38%] bg-gradient-to-br from-slate-950 via-[#0f172a] to-slate-950 text-white flex flex-col justify-between px-8 py-10 lg:px-12 lg:py-14 border-b lg:border-b-0 lg:border-r border-white/10">
        <div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-teal-300">Administrator access</p>
              <h1 className="text-xl font-black uppercase tracking-tight mt-1">Admin sign-in</h1>
            </div>
          </div>
          ...
          {view === "platform" ? (
            <div className="mt-10 max-w-md animate-in fade-in slide-in-from-bottom-2 duration-500">
              <p className="text-sm text-slate-300 leading-relaxed mb-8">
                Sign in with your administrator credentials to open the admin console.
                
              </p>
              <button
                onClick={() => setView("admin")}
                className="w-full flex items-center justify-center gap-3 bg-teal-600 hover:bg-teal-500 text-white py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.3em] transition-all shadow-lg shadow-teal-900/30"
              >
                <Shield size={16} /> Access Admin Console
              </button>
              <button
                onClick={() => { setView("register"); setError(""); }}
                className="mt-4 w-full text-[9px] font-black text-teal-300/80 uppercase tracking-[0.3em] hover:text-teal-200 transition-colors"
              >
        ...
        <p className="text-[7px] font-black text-slate-600 uppercase tracking-[0.5em] pt-16">Greggory Systems &amp; Strategy Firm © {new Date().getFullYear()}</p>
      </aside>
      ...
      <main className="relative z-10 flex-1 bg-white flex items-center justify-center px-6 py-12 sm:px-12 lg:px-16 min-h-[58vh]">
        ...
        <div className="w-full max-w-md">
            ...
            {view === "admin" && (
              <div className="animate-in fade-in slide-in-from-right-2 duration-300 w-full">
                ...
                <form onSubmit={handleLoginSubmit} className="space-y-4">
                  <div>
                    <label className="block text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Email address</label>
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-[11px] font-bold text-slate-900 outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all" placeholder="name@example.com" />
                  </div>
                  <div>
                    <label className="block text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Password</label>
                    <div className="relative">
                      <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-[11px] font-bold text-slate-900 outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all pr-12" placeholder="••••••••" />
                  ...
                  <button type="submit" disabled={loading} className="w-full bg-slate-900 hover:bg-black text-white py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.3em] shadow-2xl transition-all flex items-center justify-center gap-3 disabled:opacity-50">
                    {loading ? <Spinner size={16} tone="white" /> : "Sign in"}
                  </button>
                </form>

                <div className="mt-10 pt-8 border-t border-slate-100 text-center">
                   <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-4">Unregistered Personnel Node?</p>
                   <button onClick={() => setView("register")} className="text-[10px] font-black text-teal-600 uppercase tracking-widest hover:underline">Create an administrator account</button>
                </div>
              </div>
            )}
            ...
            {view === "register" && (
              <div className="animate-in fade-in slide-in-from-right-2 duration-300 w-full">
                <div className="text-center mb-8">
                  <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Create administrator account</h2>
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">Set up the login details for this account</p>
                </div>
                ...
                {regStep === 3 ? (
                  <div className="text-center py-10">
                    ...
                    <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-2">Account created</h3>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-8">This administrator account is ready to use.</p>
                    <button onClick={() => setView("admin")} className="w-full bg-slate-900 text-white py-4 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl">Continue to sign in</button>
                  </div>
                ) : (
            ...
            {view === "platform" && (
              <div className="text-center py-16">
                ...
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.4em]">Nothing to show yet</p>
                <p className="text-[8px] font-bold text-slate-300 uppercase tracking-widest mt-2">Choose an option on the left to continue</p>
              </div>
            )}
