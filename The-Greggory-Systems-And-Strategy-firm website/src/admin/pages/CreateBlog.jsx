      ...
      <div className="bg-[#0f172a] px-6 py-3 flex items-center justify-between border-b border-white/5 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:block">
            <h2 className="text-lg font-black text-white uppercase leading-none tracking-tighter">New blog post</h2>
            <p className="text-[6px] text-teal-500 font-black uppercase tracking-[0.4em] mt-1">Blog</p>
          </div>
        </div>
        ...
        <button onClick={() => navigate("/admin/content")} className="px-3 py-1.5 bg-rose-600/20 text-rose-500 rounded-lg text-[8px] font-black uppercase tracking-widest hover:bg-rose-700 transition-all border border-rose-600/20 flex items-center gap-2"><X size={12} /> Cancel</button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto bg-slate-950 p-6 pb-24">
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <section className="bg-white/2 p-5 rounded-2xl border border-white/5 shadow-2xl">
            <div className="flex items-center gap-2 text-teal-400 border-b border-white/5 pb-2 mb-4">
              <ImageIcon size={10} />
              <h4 className="text-[8px] font-black uppercase tracking-[0.2em]">Cover image</h4>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-4">
                <div>
                  <label className="block text-[6px] font-black text-slate-500 uppercase tracking-widest mb-1.5 px-1">Image URL</label>
                  <div className="relative">
                    <Link className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-600" size={10} />
                    <input type="url" placeholder="https://..." value={form.image_url} onChange={(e) => setForm({ ...form, image_url: e.target.value, image_base64: "" })} className="w-full bg-white/5 border border-white/10 rounded-lg pl-8 pr-3 py-2 text-[9px] font-bold text-white outline-none focus:border-teal-500" />
                  </div>
                </div>
                ...
                <div>
                   ...
                   <button type="button" onClick={() => fileInputRef.current.click()} className="w-full flex items-center justify-center gap-2 bg-white/5 border border-white/10 hover:bg-white/10 text-white rounded-lg py-2.5 transition-all"><Upload size={12} className="text-teal-400" /><span className="text-[8px] font-black uppercase tracking-widest">Upload a file</span></button>
                </div>
              </div>
              <div className={`aspect-video max-h-[140px] rounded-2xl border border-dashed border-white/10 bg-white/2 overflow-hidden transition-all duration-500 flex items-center justify-center ${form.image_url || form.image_base64 ? 'border-teal-500/30' : ''}`}>
            ...
            <div className="space-y-6">
               <section className="space-y-4 bg-white/2 p-5 rounded-2xl border border-white/5 h-full">
                  ...
                  <div className="grid gap-4">
                    ...
                    <div><label className="block text-[6px] font-black text-slate-500 uppercase tracking-widest mb-1 px-1">Author</label><div className="relative"><User className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-600" size={10} /><input type="text" value={form.author} onChange={(e) => setForm({ ...form, author: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-lg pl-8 pr-3 py-2 text-[9px] font-bold text-white outline-none" /></div></div>
                  </div>
               </section>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 bg-[#0f172a] rounded-2xl p-6 border border-white/10 shadow-2xl flex flex-col md:flex-row justify-between items-center gap-6 mt-4">
            ...
            <button type="submit" disabled={isSubmitting} className="bg-teal-600 hover:bg-teal-500 text-white px-10 py-3 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] shadow-xl flex items-center gap-3 border border-teal-400/20 transition-all active:scale-95">
              {isSubmitting ? <Spinner size={12} tone="white" /> : <Send size={12} />} Publish
            </button>
          </div>
        </div>
      </form>
