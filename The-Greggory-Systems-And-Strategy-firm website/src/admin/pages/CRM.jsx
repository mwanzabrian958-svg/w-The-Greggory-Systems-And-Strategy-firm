    ...
    <div className="space-y-6 animate-fade-in font-sans max-w-[1400px] mx-auto">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {data.pipeline.map((lead) => (
          <div key={lead.stage} className="bg-white rounded-xl p-4 border border-slate-100 shadow-md flex items-center justify-between">
            <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="bg-white rounded-2xl p-5 border border-slate-100 shadow-xl overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-50 pb-4 mb-4">
            ...
            <button className="p-1.5 bg-teal-50 text-teal-600 rounded-lg"><Plus size={14} /></button>
          </div>
                ...
                {data.clients.length > 0 ? data.clients.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                    <td className="py-3 px-1">
                      <p className="text-[9px] font-black text-slate-900 uppercase">{c.name}</p>
        ...
        <section className="bg-[#0f172a] rounded-2xl p-5 border border-white/5 shadow-2xl">
          ...
          <div className="space-y-3">
             {data.opportunities.length > 0 ? data.opportunities.map((opp) => (
               <div key={opp.id} className="p-3 bg-white/5 rounded-xl border border-white/5 hover:bg-white/10 transition-all cursor-pointer group">
                  ...
                  <div className="flex justify-between items-end">
                     <p className="text-sm font-black text-white">KSh {opp.value?.toLocaleString()}</p>
