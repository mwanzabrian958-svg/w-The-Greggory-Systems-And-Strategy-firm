...
import {
  ...
  Download, Settings, Search, X, ChevronRight, ChevronDown,
...
const StatusBadge = ({ status }) => {
  const map = {
    ...
    'in-progress': 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20 shadow-[0_0_10px_rgba(16,185,129,0.2)]',
...
const SectionHeader = ({ title, subtitle, action }) => (
  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
    <div>
      ...
      {subtitle && <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest">{subtitle}</p>}
    </div>
    {action}
  </div>
)
...
// SECTION: OVERVIEW
// ─────────────────────────────────────────────
const OverviewSection = ({ data = null, notifs = [], isLoading = false }) => {
...
// SECTION: PROJECTS
// ─────────────────────────────────────────────
const ProjectsSection = ({ projects = [], onView, onEdit }) => {
    ...
    <div className="space-y-8">
      ...
      {projects.length === 0 ? (
        <div className="text-center py-20 border-2 border-dashed border-white/5 rounded-[40px]">
           <Layers className="w-12 h-12 text-slate-800 mx-auto mb-4" />
           <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">No Real-Time Projects Synced</p>
        </div>
      ) : (
        <>
          {viewMode === 'kanban' ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 overflow-x-auto pb-4">
              {['planning', 'in-progress', 'completed'].map(col => (
                <div key={col} className="bg-slate-50 dark:bg-white/5 backdrop-blur-xl rounded-[32px] p-6 border border-slate-200 dark:border-white/10 min-w-[320px]">
                  <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-3">
                       <div className={`w-2 h-2 rounded-full ${col === 'in-progress' ? 'bg-emerald-500' : col === 'completed' ? 'bg-blue-500' : 'bg-gold-500'}`}></div>
                      ...
                      <div key={p.id} className="bg-white dark:bg-white/[0.02] rounded-[24px] p-6 border border-slate-100 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/[0.05] hover:border-slate-300 dark:hover:border-white/20 transition-all cursor-pointer group shadow-sm dark:shadow-none" onClick={() => onView && onView(p)}>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex items-center justify-between pt-2">
                            <Avatar initials={p.manager?.split(' ').map(n => n[0]).join('') || 'M'} size="sm" />
                            <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">ETA: {p.deadline ? formatTime(p.deadline) : 'TBD'}</span>
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-10">
                        ...
                        <div className="flex justify-between text-[9px] font-black uppercase tracking-widest text-slate-600 mt-4">
                          <span>INIT: {p.createdAt ? formatTime(p.createdAt) : 'TBD'}</span>
                          <span>TERM: {p.deadline ? formatTime(p.deadline) : 'TBD'}</span>
                        </div>
                      </div>
                    </div>
...
// SECTION: BILLING
// ─────────────────────────────────────────────
const BillingSection = ({ projects = [], invoices = [], budget = null }) => {
  const totalBudget = budget?.planned || 0
  const totalSpent = budget?.spent || 0
  const remaining = totalBudget - totalSpent
  const utilPct = budget?.planned > 0 ? Math.round((totalSpent / totalBudget) * 100) : 0

  return (
    <div className="space-y-10">
      <SectionHeader title="Billing Ledger" subtitle="Capital allocation and burn matrix telemetry" />

      {projects.length === 0 ? (
        <div className="text-center py-20 border-2 border-dashed border-white/5 rounded-[40px]">
           <DollarSign className="w-12 h-12 text-slate-800 mx-auto mb-4" />
           <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Zero Billing Frequencies</p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <SectionCard className="p-10 border border-white/10">
              <h3 className="text-[10px] font-black text-white uppercase tracking-[0.3em] mb-10">Entity Allocation Tracker</h3>
              <div className="space-y-8">
                {projects.map(p => (
                  <div key={p.id}>
...
const DocumentsSection = ({ documents = [] }) => {
  ...
  const filtered = activeCategory === 'All' ? documents : documents.filter(d => (d.category || "").toLowerCase() === activeCategory.toLowerCase())

  return (
    <div className="space-y-10">
      <SectionHeader title="Secure Repository" subtitle="Centralized vault for all strategic documentation" />
      ...
      {documents.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filtered.map(doc => (
            <SectionCard key={doc.id} className="p-8 group hover:bg-white/[0.08] border border-white/10 relative overflow-hidden">
              <div className="flex items-start gap-6 relative z-10">
                <div className="p-5 bg-white/5 rounded-2xl text-gold-500 border border-white/10"><FileText className="w-6 h-6" /></div>
                <div className="flex-1 min-w-0">
                  <p className="font-black text-white text-sm uppercase truncate">{doc.name}</p>
                ...
                <button onClick={() => toast('Initializing Download…')} className="p-3 bg-white/5 hover:bg-white/10 rounded-xl text-slate-400 hover:text-gold-500 transition-all"><Download size={16} /></button>
              </div>
            </SectionCard>
          ))}
        </div>
...
// ─────────────────────────────────────────────
const CommunicationSection = ({ messages = [] }) => {
  const [msg, setMsg] = useState('')

  return (
    <div className="space-y-10">
      <SectionHeader title="Communication Hub" subtitle="Secure discussion threads and tactical synchronization" />

      {messages.length === 0 ? (
        <div className="text-center py-20 border-2 border-dashed border-white/5 rounded-[40px]">
           <MessageSquare className="w-12 h-12 text-slate-800 mx-auto mb-4" />
           <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Discussion Threads Empty</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          <div className="space-y-4">
            {messages.map(m => (
              <SectionCard key={m.id} className={`p-8 group hover:bg-white/[0.08] border border-white/10 ${m.unread ? 'border-l-4 border-l-gold-500' : ''}`}>
                <div className="flex items-start gap-6">
                  <Avatar initials={m.sender.split(' ').map(n => n[0]).join('')} size="md" color={m.unread ? 'bg-gold-500' : 'bg-white/10'} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-4 mb-3">
                      <span className="font-black text-white text-sm uppercase">{m.sender}</span>
                      <span className="text-[10px] font-black text-slate-600 uppercase">{formatTime(m.time)}</span>
                    </div>
                    <p className="text-[10px] font-black text-gold-500 uppercase tracking-widest mb-2">{m.subject}</p>
          ...
          <SectionCard className="p-8 border border-white/10 h-fit">
    ...
    {notifs.length === 0 ? (
      ...
      <div className="space-y-4 max-w-4xl">
        {notifs.map(n => {
          ...
          const isRead = n.status === 'read'
          return (
            <div key={n.id} onClick={() => !isRead && onToggleRead(n.id)} className={`flex items-start gap-6 p-6 rounded-[24px] border transition-all ${isRead ? 'bg-white/[0.01] border-white/5 opacity-60' : 'bg-white/[0.03] border-gold-500/20 shadow-lg cursor-pointer'}`}>
              <div className={`p-4 rounded-2xl bg-white/[0.02] border border-white/10 shadow-sm ${color}`}><Icon className="w-5 h-5" /></div>
              <div className="flex-1">
                <p className={`text-sm font-black uppercase ${isRead ? 'text-slate-300' : 'text-white'}`}>{n.title}</p>
                <p className="text-xs text-slate-500 font-medium leading-relaxed mt-2 uppercase tracking-widest">{n.message}</p>
                <p className="text-[10px] font-black text-slate-600 uppercase mt-4 flex items-center gap-2"><Clock size={10} />{formatTime(n.created_at)}</p>
              </div>
            </div>
          )
        })}
...
const FeedbackSection = ({ user }) => {
  ...
  const submitSatisfaction = async () => {
    if (rating === 0) {
      toast('Select a satisfaction level', 'warn')
      return
    }
  ...
  const submitIncident = async () => {
    if (!incidentMsg.trim()) {
      toast('Enter incident details', 'warn')
      return
    }
    setIsSubmitting(true)
    try {
      const response = await fetch(getApiUrl('/api/feedback'), {
        ...
        body: JSON.stringify({
          ...
          message: incidentMsg,
          title: 'Tactical Incident Report',
          priority: 'High'
        })
      })
      const data = await response.json()
      if (data.success) {
        toast('Incident alert initialized', 'success')
        setIncidentMsg('')
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              >
                <Star
                  className={`w-10 h-10 transition-colors ${
                    (hoverRating || rating) >= s ? 'text-gold-500 fill-gold-500' : 'text-slate-800'
                  }`}
                />
              </button>
            ))}
        ...
        <SectionCard className="p-8">
          <h3 className="font-black text-white uppercase text-sm mb-6 flex items-center gap-3"><AlertTriangle className="w-5 h-5 text-rose-500" />Incident Report</h3>
          <p className="text-xs font-medium text-slate-500 uppercase tracking-[0.2em] mb-4">Report tactical disruptions or system anomalies.</p>
          <textarea
            value={incidentMsg}
            onChange={(e) => setIncidentMsg(e.target.value)}
            placeholder="Describe the anomaly..."
            className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-rose-500/50 mb-6 h-24 resize-none"
          />
          <button
...
const RiskSection = () => (
  <div className="space-y-10">
    <SectionHeader title="Risk & Quality" subtitle="Mitigation register and quality assurance telemetry" />
    <div className="text-center py-20 border-2 border-dashed border-white/5 rounded-[40px]">
       <Shield className="w-12 h-12 text-slate-800 mx-auto mb-4" />
       <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Strategic Risk Matrix Clear</p>
    </div>
  </div>
)
...
const AnalyticsSection = () => (
  <div className="space-y-10">
    <SectionHeader title="Analytics" subtitle="Deep-space data visualization and performance metrics" />
    <div className="text-center py-20 border-2 border-dashed border-white/5 rounded-[40px]">
       <BarChart2 className="w-12 h-12 text-slate-800 mx-auto mb-4" />
       <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Neural Data Analysis Pending</p>
    </div>
  </div>
)
...
const SecuritySection = ({ user }) => (
  <div className="space-y-10">
    <SectionHeader title="Security Protocol" subtitle="Identity management and access logs" />
    <SectionCard className="p-8 border border-white/10">
       <div className="flex items-center gap-6 mb-8">
          <div className="p-5 rounded-2xl bg-emerald-500/10 text-emerald-500"><Lock size={24} /></div>
          <div>
            <p className="text-lg font-black text-white uppercase tracking-tight">Active Encryption Enabled</p>
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">Status: High-Security Node</p>
          </div>
       </div>
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-6 rounded-2xl bg-white/5 border border-white/5"><p className="text-[9px] font-black text-slate-500 uppercase mb-2">Auth Level</p><p className="text-sm font-black text-white uppercase">{user?.role || 'Standard Agent'}</p></div>
          <div className="p-6 rounded-2xl bg-white/5 border border-white/5"><p className="text-[9px] font-black text-slate-500 uppercase mb-2">Last Sync</p><p className="text-sm font-black text-white uppercase">Just Now</p></div>
       </div>
    </SectionCard>
  </div>
)
...
// ─────────────────────────────────────────────
// SECTION: ROADMAP, REPORTS
// ─────────────────────────────────────────────
const RoadmapSection = () => (
  <div className="space-y-10">
    <SectionHeader title="Strategic Roadmap" subtitle="Timeline of operational milestones" />
    <div className="text-center py-20 border-2 border-dashed border-white/5 rounded-[40px]">
       <Map className="w-12 h-12 text-slate-800 mx-auto mb-4" />
       <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Timeline Trajectory Clear</p>
    </div>
  </div>
)
...
const ReportsSection = () => (
  <div className="space-y-10">
    <SectionHeader title="Reports" subtitle="Generated strategic and financial documentation" />
    <div className="text-center py-20 border-2 border-dashed border-white/5 rounded-[40px]">
       <Download className="w-12 h-12 text-slate-800 mx-auto mb-4" />
       <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Archive Empty: No Reports Generated</p>
    </div>
  </div>
)
...
const SettingsSection = ({ user }) => {
  ...
  return (
    <div className="space-y-10">
      <SectionHeader title="Account Settings" subtitle="Preferences and account node parameters" />

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <SectionCard className="p-10">
          <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-10">Profile information</h3>
          <div className="space-y-6">
            <div>
              <label className="text-[9px] font-black text-slate-500 uppercase mb-2 block">Agent Name</label>
              <input value={user?.display_name || user?.name || ''} disabled className="w-full bg-white/5 border border-white/5 rounded-2xl px-6 py-4 text-xs font-black uppercase text-slate-500" />
            </div>
            <div>
              <label className="text-[9px] font-black text-slate-500 uppercase mb-2 block">Secure Email</label>
              <input value={user?.email || ''} disabled className="w-full bg-white/5 border border-white/5 rounded-2xl px-6 py-4 text-xs font-black uppercase text-slate-500" />
            </div>
          </div>
        </SectionCard>
        ...
        <div className="space-y-6">
          <SectionCard className="p-10">
            <h3 className="text-[10px] font-black text-slate-500 uppercase mb-8">Notification relays</h3>
            <div className="space-y-6">
              {[
                ...
                <div key={pref.label} className="flex items-center justify-between group">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{pref.label}</span>
                  <button onClick={() => pref.setter(p => !p)}
                    className={`relative w-12 h-6 rounded-full transition-all border ${pref.val ? 'bg-gold-500 border-gold-500' : 'bg-white/5 border-white/10'}`}>
                    <div className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full transition-transform ${pref.val ? 'translate-x-6 bg-slate-950' : 'bg-slate-600'}`} />
                  </button>
                </div>
              ))}
            </div>
          ...
          <SectionCard className="p-10">
            <h3 className="text-[10px] font-black text-slate-500 uppercase mb-8">Visual protocol</h3>
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Dark mode</span>
              <button onClick={toggleTheme}
                className={`relative w-12 h-6 rounded-full transition-all border ${darkMode ? 'bg-gold-500 border-gold-500' : 'bg-white/5 border-white/10'}`}>
                <div className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full transition-transform ${darkMode ? 'translate-x-6 bg-slate-950' : 'bg-slate-600'}`} />
              </button>
            </div>
          </SectionCard>
        </div>
...
const Projects = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (dashRes.status === 401) { logout(); return }
      const dashData = await dashRes.json()
      if (dashData.success) setDashboardData(dashData.dashboard)
      const notifRes = await fetch(getApiUrl('/api/users/notifications/me'), { headers })
      const notifData = await notifRes.json()
      if (notifData.success) setNotifs(notifData.notifications)
    } catch (error) {
  ...
  const toggleNotifRead = async (id) => {
    ...
    try { await fetch(getApiUrl(`/api/users/notifications/${id}/read`), { method: 'PUT', headers: { 'Authorization': `Bearer ${token}` } }) }
    catch (error) { console.error('Acknowledgement relay failure:', error) }
  }

  const acknowledgeAllNotifs = async () => {
    setNotifs(prev => prev.map(n => ({ ...n, status: 'read' })))
    try {
      await fetch(getApiUrl('/api/users/notifications/read-all/me'), { method: 'PUT', headers: { 'Authorization': `Bearer ${token}` } })
      toast('All relays acknowledged', 'success')
    } catch (error) { console.error('Command frequency error:', error) }
  }
      ...
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-4">
        <div className="text-center max-w-lg w-full relative">
          ...
          <div className="relative z-10 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[32px] p-10 shadow-2xl">
            ...
            <h2 className="text-2xl font-black text-white uppercase tracking-tight mb-4">Access Restricted</h2>
            <p className="text-slate-400 font-medium leading-relaxed mb-8">Protocol requires authentication to access the Client Portal.</p>
            <Link to="/login" className="block w-full py-4 bg-gold-500 text-slate-950 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] hover:bg-gold-400 transition-all shadow-xl shadow-gold-500/20 text-center">Initialize Secure Login</Link>
          </div>
        </div>
      </div>
    )
  ...
  const renderSection = () => {
    ...
    switch (activeSection) {
      case 'overview':       content = <OverviewSection data={dashboardData} notifs={notifs} isLoading={isLoading} />; break
      case 'projects':       content = <ProjectsSection projects={dashboardData?.projects} onView={p => openModal('viewProject', p)} onEdit={p => openModal('editProject', p)} />; break
      case 'billing':     content = <BillingSection projects={dashboardData?.projects} invoices={dashboardData?.invoices} budget={dashboardData?.budgetOverview} />; break
      case 'documents':      content = <DocumentsSection documents={dashboardData?.documents} />; break
      case 'communication':  content = <CommunicationSection messages={dashboardData?.messages} />; break
      case 'notifications':  content = <NotificationsSection notifs={notifs} isLoading={isLoading} onToggleRead={toggleNotifRead} onRefresh={fetchData} />; break
      case 'feedback':       content = <FeedbackSection user={user} />; break
      ...
      default:               content = <OverviewSection data={dashboardData} notifs={notifs} isLoading={isLoading} />; break
    }
    return <div className="animate-in fade-in slide-in-from-bottom-2 duration-700 fill-mode-both">{content}</div>
  }
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] flex flex-col overflow-hidden text-[var(--text-primary)] font-medium relative transition-colors duration-500">
      ...
      <header className="bg-white/80 dark:bg-white/5 backdrop-blur-3xl border-b border-slate-200 dark:border-white/10 px-6 sm:px-10 py-4 flex flex-col md:flex-row md:items-center justify-between gap-6 flex-shrink-0 relative z-[100] shadow-xl dark:shadow-2xl transition-colors">
        <div className="flex items-center gap-6 flex-grow overflow-hidden">
          <div className="flex items-center gap-3 overflow-x-auto no-scrollbar pb-1 flex-grow">
          ...
          <div className="hidden lg:block border-l border-slate-200 dark:border-white/10 pl-6 flex-shrink-0 text-right">
            <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em]">{currentNav?.label || 'Portal'}</h2>
            <p className="text-sm font-black text-slate-900 dark:text-white tracking-tight">Greggory portal</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => navigate('/')} className="p-3 rounded-2xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:bg-rose-500 hover:text-white transition-all text-slate-500 dark:text-slate-400 group" title="Close portal"><X size={24} /></button>
        </div>
      </header>

      <main className="flex-1 overflow-y-auto p-6 sm:p-10 relative z-10 custom-scrollbar">
